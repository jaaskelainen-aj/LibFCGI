...
struct B4Num
{
    ...
    uint8_t B1;
    uint8_t B0;
    B4Num() { B3=0; B2=0; B1=0; B0=0; }
    explicit B4Num(uint32_t val) { B3=val>>24; B2=(val>>16)&0xff; B1=(val>>8)&0xff; B0=val&0xff; }
...
struct B2Num
{
    ...
    void set(uint16_t val) { B1 = val>>8; B0=val&0xff; }
...
struct Header
{
    ...
    Header() : version(1), type(TYPE_UNKNOWN), padding_length(0), reserved(0) {}
    Header(message_type_t t, uint16_t id)
            : version(1), type(t), request_id(id), padding_length(0), reserved(0) { }
    Header(message_type_t t, uint16_t id, uint16_t size)
            : version(1), type(t), request_id(id), content_length(size), padding_length(0), reserved(0) { }
};
...
struct BeginRequestMsg
{
    ...
    uint8_t flags;
    uint8_t reserved[5];
    BeginRequestMsg() : flags(0) { memset(reserved,0,sizeof(reserved)); }
};
...
struct EndRequestMsg : public Header
{
    ...
    EndRequestMsg() : protocol_status(0) { memset(reserved,0,sizeof(reserved)); }
