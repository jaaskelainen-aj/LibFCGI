...
#else
    uint64_t getKey(size_t ndx)      { if(ndx<key_count) return key_array[ndx]; return 0; }
    const char* getValue(size_t ndx) { if(ndx<key_count) return value_ptr[ndx]; return &dummy; }
#endif
