...
// -------------------------------------------------------------------------------------------------
void Request::log(const char *str)
{
    TRACE(str);
}
// -------------------------------------------------------------------------------------------------
Request::Request()
    : rbin(Request::input_size), params(Request::param_size)
{
    role = RESPONDER;
    fd_spool   = -1;
    mp_count   = 0;
...
// -------------------------------------------------------------------------------------------------
void Request::setPollFd(pollfd *fd)
{
    if(fd) {
...
// -------------------------------------------------------------------------------------------------
void Request::write(const char *data, uint16_t length)
{
    // Check the validity
...
bool Request::writeFd(int fd)
{
    ...
    TRACE("Request::writeFd (%d) - from %d; max %ld; in fd %ld; pending %d\n",
          id, pfd.fd, original_max, total, (int)(rbpos-rbout));
    flush();
    return true;
}
// -------------------------------------------------------------------------------------------------
...
// -------------------------------------------------------------------------------------------------
void Request::end(uint32_t _app_status)
{
    if(state != RQS_OPEN) {
...
// -------------------------------------------------------------------------------------------------
void Request::processTestSpool()
{
    processSpool();
}
// -------------------------------------------------------------------------------------------------
bool Request::openMPSpool(const char *data, int len)
{
    ...
    if(fd_spool == -1) {
        CS_VAPRT_ERRO("Request::openMPSpool - unable to open spool file for multipart imput. Errno %d.",errno);
        end(500);
        return false;
    }
...
// -------------------------------------------------------------------------------------------------
void Request::writeSpool(uint16_t msg_len, const char *msg)
{
    // If msg is null, rbin is used !

    if( fd_spool > 0) {
        TRACE("Request::writeSpool (%d) - disc spool %d bytes\n", id, msg_len);
        if(msg) ::write(fd_spool, msg, msg_len);
        else rbin.read_into(fd_spool, msg_len);
        spool_size += msg_len;
        return;
    }
    if( spool_size+msg_len < REQ_MAX_MEMSTDIN) {
        TRACE("Request::writeSpool (%d) - memory spool %d bytes\n", id, msg_len);
        if(msg) strncpy(stdin_buffer+spool_size, msg, msg_len);
        else rbin.read(stdin_buffer+spool_size, msg_len);
        spool_size += msg_len;
        return;
    }
    ...
    fd_spool = open(spool_path, O_RDWR|O_CREAT|O_TRUNC, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH);
    if(fd_spool == -1) {
        CS_VAPRT_ERRO("Request::writeSpool - unable to open spool file for stdin imput. Errno %d.",errno);
        end(500);
        return;
    }
    mp_count++;
...
#ifdef UNIT_TEST
            fprintf(trace, "Request::processSpool - Unable to process stdin form, syntax error in:\n");
...
#else
        CS_PRINT_WARN("Request::processSpool - Unable to process stdin form, parameter syntax error 2. "\
                      "Aborting request.");
#endif
        return false;
    }
...
// -------------------------------------------------------------------------------------------------
size_t Request::parseMultipart(char *data, size_t dlen, ParseData *pd)
{
    char *ptr = data;
    char *end = ptr + dlen;
...
// -------------------------------------------------------------------------------------------------
bool Request::createXferFile(ParseData *pd)
{
    time_t now;
    struct tm *nowtm;
    char datestamp[20];
    ...
    if(upload_ndx < REQ_MAX_UPLOADS) {
        ...
        if(driver) {
            sprintf(uploads[upload_ndx]->internal, "%supload%s_%ld",
                    driver->upload_path.c_str(), datestamp, filendx++);
        }
        else {
    ...
    pd->fd_upload = open(uploads[upload_ndx]->internal,
...
// -------------------------------------------------------------------------------------------------
void Request::processBeginRequest(uint32_t ndx)
{
    ...
    if( (beg_req.flags & FLAG_KEEP_CONN)>0) flags.set(FLAG_KEEP);
    else flags.clear(FLAG_KEEP);

    role = (role_t)beg_req.role.get();
}

// -------------------------------------------------------------------------------------------------
void Request::parseRequestMethod(NameValue *nv)
{
    char type[9];
    PACK64 pack;
...
/* To save time in comparing parameter names we have pre-calculated 64bit has values for them.
  */
void Request::processParams(uint64_t *plimit_hash_list, uint16_t msg_len)
{
    bool accepted=true;
    ...
    while(processed < msg_len)
    {
        ...
        if(nv.name_len >= DRIVER_PARAMNAME-1) {
...
#ifdef UNIT_TEST
            {
                ...
                size_t max =  sizeof(value_buf) > nv.value_len ? nv.value_len : sizeof(value_buf)-1;
            ...
            case HASH_CONTENT_TYPE:
                if(nv.value_len < sizeof(content)) {
                    ...
                    if(!strncmp(content, "multipart/form-data",19)) {
                        ...
                        if(boundary_ptr) {
                            openMPSpool(boundary_ptr+9, nv.value_len - (boundary_ptr - content) - 9);
                        } else {
#ifdef UNIT_TEST
                            TRACE("Request::process_params - multipart boundary not found - content:%s\n",
                                  content);
#else
                            CS_PRINT_ERRO("Request::process_params - multipart boundary not found.");
#endif
                        }
                    }
                    ...
                    CS_VAPRT_WARN("Request::process_params - content type value too long (%d) "\
                                  "for the internal buffer. Ignored.",nv.value_len);
                    rbin.discard(nv.value_len);
                }
                break;
            ...
            case HASH_REQUEST_URI:
                ...
                if(max<nv.value_len) {
                    ...
                    CS_VAPRT_WARN("Request::process_params - URI longer than reserverd space. URI len=%d",
                                  nv.value_len);
                }
                break;
...
// -------------------------------------------------------------------------------------------------
void Request::processStdin(uint16_t msg_len)
{
