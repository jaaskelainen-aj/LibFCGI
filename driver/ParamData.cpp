...
// -------------------------------------------------------------------------------------------------
void ParamData::resize(size_t add_size)
{
...
// -------------------------------------------------------------------------------------------------
void ParamData::init_push(size_t addsize)
{
    callback_state = KEY;
    resize(addsize);
...
// -------------------------------------------------------------------------------------------------
void ParamData::push_back(char ch)
{
...
// -------------------------------------------------------------------------------------------------
void ParamData::end_push()
{
    if(callback_state!=VALUE) {
        char tbuffer[128];
        sprintf(tbuffer,"ParamData::end_push - Key-value-pair not completed. State=%d, key_len=%ld\n",callback_state, key_ndx);
...
// -------------------------------------------------------------------------------------------------
uint64_t ParamData::add(const char *key, size_t keysize, const char *value)
{
...
// -------------------------------------------------------------------------------------------------
char* ParamData::add(uint64_t hash, size_t valsize)
{
...
// -------------------------------------------------------------------------------------------------
uint64_t ParamData::add(uint64_t key, const char *value)
{
...
// -------------------------------------------------------------------------------------------------
const char *ParamData::get(const char *key, size_t keylen)
{
...
// -------------------------------------------------------------------------------------------------
bool ParamData::get(const char *key, size_t keylen, int &value)
{
    size_t ndx;
    char *dummy;
    if(!key_count || !key || !key[0] || !keylen)
...
// -------------------------------------------------------------------------------------------------
bool ParamData::find(const char *key, size_t keylen)
{
    if(!key_count || !key || !key[0] || !keylen)
        return false;
    uint64_t k64 = fnv_64bit_hash(key,keylen);
...
// -------------------------------------------------------------------------------------------------
const char* ParamData::get(uint64_t k64)
{
    size_t ndx;
    if(!key_count)
        return &dummy;
...
#ifdef _DEBUG
// -------------------------------------------------------------------------------------------------
uint64_t ParamData::getKey(size_t ndx)
{
    if(ndx<key_count)
...
// -------------------------------------------------------------------------------------------------
const char* ParamData::getValue(size_t ndx)
{
    if(ndx<key_count)
        return value_ptr[ndx];
    return &dummy;
...
// -------------------------------------------------------------------------------------------------
bool ParamData::findFirst(const char *subkey, size_t *ndx)
{
    PACK64 pack;
    pack.value = 0;
    find_ndx = 0;
...
bool ParamData::findNext(size_t *ndx)
{
    if(!ndx)
        return false;
    uint64_t mask = 0xFF;
