...
void
AppStr::ASArray::add(size_t index, const char* str, size_t line)
{
    if (index >= count) {
        std::ostringstream err;
        err << "AppStr::ASArray::add - Line=" << line << ". Index out of bounds:" << std::hex
            << index;
...
const char*
AppStr::ASArray::get(uint16_t ndx)
{
    if (ndx >= count)
        return NO_MSG;
    if (!strarr[ndx])
...
void
AppStr::ASGroup::add(size_t index, ASArray* array, size_t line)
{
    char errbuf[156];
    if (index >= size) {
...
const char*
AppStr::ASGroup::getCPtr(uint16_t ndx)
{
    uint8_t grp = ndx >> 8;
        ...
        for (int i = 0; i < bytes; i++) {
            ...
            case STATE_LF:
                if (fcgi_driver::isHex(buffer[i])) {
                    ...
                    close(msg_file);
                    sprintf(errbuf,
                            "AppStr::AppStr - Syntax error: Unrecognized characters. Line %ld",
                            line);
                    throw runtime_error(errbuf);
                }
                line++;
            ...
            case STATE_NUMBER:
                if (!fcgi_driver::isHex(buffer[i])) {
                    ...
                    if (first) {
                        // ++++++++++++++++++++++++++++++++++++++++++++++++++
                        // cout << "Found msg - "<<number<<"; "<<listIndex<<"; "<<(int)group_no<<";
                        // "<<(int)str_no<<'\n';
                        ...
                        if (group_prev > group_no) {
                            close(msg_file);
                            sprintf(errbuf,
                                    "AppStr::AppStr - Syntax error: Groups should be sequentally "
                                    "ordered within a file. Line %ld",
                                    line);
                            throw runtime_error(errbuf);
                        }
                        if (group_prev < group_no) {
                            // ++++++++++++++++++++++++++++++++++++++++++++++++++
                            // cout << "New group "<<(int)group_prev<<" with "<<strings->size()<<"
                            // strings. Next "<<(int)group_no<<'\n';
                    ...
                    if (nIndex == sizeof(number)) {
                        close(msg_file);
                        sprintf(errbuf,
                                "AppStr::AppStr - Syntax error: String number too large. Line %ld",
                                line);
                        throw runtime_error(errbuf);
                    }
                }
            ...
            case STATE_WS:
                if (buffer[i] == '"') {
                    ...
                    close(msg_file);
                    sprintf(errbuf,
                            "AppStr::AppStr - Syntax error: Number defined but start of string "
                            "missing. Line %ld",
                            line);
                    throw runtime_error(errbuf);
                }
...
// -------------------------------------------------------------------------------------------------
APPSTR_LC
AppStr::lcname2code(const char* locname)
{
    int ndx;
...
// -------------------------------------------------------------------------------------------------
const char*
AppStr::code2lcname(APPSTR_LC lc)
{
