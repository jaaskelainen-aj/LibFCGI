...
const char url64_table[] = { "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                             "abcdefghijklmnopqrstuvwxyz"
                             "0123456789-_" };
const char smtp64_table[] = { "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                              "abcdefghijklmnopqrstuvwxyz"
                              "0123456789+/" };
...
// -------------------------------------------------------------------------------------------------
size_t
base64_encode(char* target, size_t t_len, const unsigned char* data, size_t in_len, Base64Type bt)
{
    size_t i = 0, j = 0, max;
...
// -------------------------------------------------------------------------------------------------
size_t
base64_decode(unsigned char* target, size_t t_len, const char* data, Base64Type bt)
{
    size_t i = 0, j = 0;
    ...
    if (data[input_length - 1] == filler)
        output_length--;
    if (data[input_length - 2] == filler)
        output_length--;
    ...
    while (data[i]) {
        ...
        if (j < output_length)
            target[j++] = (triple >> 2 * 8) & 0xFF;
        if (j < output_length)
            target[j++] = (triple >> 1 * 8) & 0xFF;
        if (j < output_length)
            target[j++] = (triple >> 0 * 8) & 0xFF;
    }
