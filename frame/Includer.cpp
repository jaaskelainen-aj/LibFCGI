...
// -------------------------------------------------------------------------------------------------
const char* Includer::getPath(fcgi_driver::Request *req, const char *fname)
{
...
// -------------------------------------------------------------------------------------------------
const char* Includer::getEditPath(fcgi_driver::Request *req, const char *name)
{
    static char inc_name[150];
    SessionBase *sb = (SessionBase*)req->app_data;
    strcpy(inc_name,docroot.c_str());
...
// -------------------------------------------------------------------------------------------------
bool Includer::exists(fcgi_driver::Request *req, const char *fname)
{
    c4s::path incfile(getPath(req,fname));
    return incfile.exists();
}
// -------------------------------------------------------------------------------------------------
void Includer::remove(fcgi_driver::Request *req, const char *fname)
{
    c4s::path(getPath(req,fname)).rm();
}
// -------------------------------------------------------------------------------------------------
void Includer::editable(fcgi_driver::Request *req, const char *name, HtmlBuffer *hb, bool rights)
{
    SessionBase *sb = (SessionBase*)req->app_data;
    if(rights)
        hb->html<<"<div id='frame_"<<name<<"' class='editor-frame' data-toggle='popover'><div data-editor_id='"<<name<<"' class='myeditors'>\n";
    hb->bufferFlush(req);
...
// -------------------------------------------------------------------------------------------------
bool Includer::toRequest(const char *fname, fcgi_driver::Request *req, bool localize)
{
// #ifdef _DEBUG
//     char dbuffer[128];
...
// #endif
    ...
    if(fd<0) {
        if(errno != ENOENT) {
            CS_VAPRT_ERRO("Includer::toRequest - Unable to open '%s'. System error:%d",fname,errno);
        }
...
// -------------------------------------------------------------------------------------------------
bool Includer::toHtml(const char *fname, ostringstream &html)
{
...
// -------------------------------------------------------------------------------------------------
void Includer::writeSafeHtml(int fd, const char *txt)
{
    // NOTE! Do not send JS Editor text here! Only short user editable text that is not supposed to have any html.
    while(*txt) {
        switch(*txt) {
        case '&' : write(fd, "&amp;" , 5); break;
        case '\'': write(fd, "&apos;", 6); break;
        case '\"': write(fd, "&quot;", 6); break;
        case '<' : write(fd, "&lt;", 4);   break;
        case '>' : write(fd, "&g;t", 4);   break;
        default:
            write(fd, txt, 1);
        }
        txt++;
...
// -------------------------------------------------------------------------------------------------
bool Includer::save(fcgi_driver::Request *req, const char *fname, const char *content, const char *title)
{
    const char *fpath = getPath(req,fname);

    int fd = open(fpath, O_WRONLY|O_TRUNC|O_CREAT, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH);
    if(fd<0) {
        CS_VAPRT_ERRO("Includer::save -- Error %d. Unable to open/create file: %s",errno, fname);
        return false;
    }
    if(fchmod (fd, S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH) == -1) {
        CS_VAPRT_ERRO("Includer::save -- Error %d. Unable to change permissions for: %s",errno, fname);
        return false;
    }
    if(title) {
        write(fd, "<!--TITLE:", 10);
