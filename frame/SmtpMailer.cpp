...
void
SmtpMsg::addRecipient(const char* rp)
{
...
bool
SmtpMsg::initializeSend(SmtpMailer* _m)
{
    mailer = _m;
    ...
    size_t rv = base64_encode(base64, encoded_size, (const unsigned char*)body_buffer, body.tellp(),
                              Base64Type::URL);
    if (rv == 0) {
...
// -------------------------------------------------------------------------------------------------
size_t
SmtpMsg::copySmtpData(void* ptr, size_t max)
{
    if (data_sent)
        return 0;
    if (!encoded_size) {
...
#ifdef UNIT_TEST
int
SmtpMailer::perform_UT(SmtpMsg* msg)
{
    const int BS = 512;
    char ptr[BS];
...
#ifdef _DEBUG
                            CS_VAPRT_ERRO("SmtpMailer::perform - Transfer #%d ready. Error: %d. %s",
                                          ndx, cmsg->data.result, transfers[ndx]->errormsg);
#else
                            CS_VAPRT_ERRO("SmtpMailer::perform - Transfer #%d ready. Error: %s",
                                          ndx, curl_easy_strerror(cmsg->data.result));
#endif
                        delete transfers[ndx];
                        transfers[ndx] = 0;
                    }
...
// -------------------------------------------------------------------------------------------------
char*
SmtpMailer::getScratchBuffer(size_t size)
{
