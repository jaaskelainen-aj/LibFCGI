...
// -------------------------------------------------------------------------------------------------
SessionMgr::SessionMgr(const string &exe_dir, const c4s::path &keyfile, SessionFactoryIF *SIF, c4s::configuration *conf)
{
    ...
    if(conf->get_value("LibFCGI","SessionLogFacility", CONF_facility)) {
        if(CONF_facility>=0 && CONF_facility<=7)
            ...
            syslog(LOG_WARNING, "SessionMgr::SessionMgr - Unknown session syslog facility. Using LOCAL4");
        }
    }
    else
        CONF_facility = 0;
    ...
    if(!conf->get_value("LibFCGI","Locales",loc_list)) {
        ...
        if(!loc_list.empty()) locales = new MultiStr(loc_list.c_str(),","," ");
        else {
    ...
    if(locales) {
        bool first=true;
        int ndx=0;
        for(const char *locid=locales->getFirst(); locid && ndx<FRAME_LOCALES; locid=locales->getNext()) {
...
// -------------------------------------------------------------------------------------------------
const char* SessionMgr::getSID()
{
    struct { time_t tt; uint64_t key; } pack;
    ...
    sid_buf[FRAME_SID]=0;
    if(CONF_facility) syslog(LOG_MAKEPRI(CONF_facility, LOG_NOTICE), "SessionMgr::getSID - key_ndx:%d time:%ld SID:%s\n",key_ndx, pack.tt, sid_buf);
    return sid_buf;
}
// -------------------------------------------------------------------------------------------------
SessionBase* SessionMgr::initializeSession(fcgi_driver::Request *req)
{
    SessionBase *base = 0;
    const char *sid = 0;
    ...
    if(req->is(fcgi_driver::FLAG_COOKIE)) {
        ...
        if(CONF_facility)
            syslog(LOG_MAKEPRI(CONF_facility, LOG_NOTICE),"SessionMgr::initializeSession - Cookie sid:%s\n",sid);
    }
    else if(req->is(fcgi_driver::FLAG_MENACON_SID)) {
        sid = req->params.get(fcgi_driver::HASH_MENACON_SID);
        if(CONF_facility)
            syslog(LOG_MAKEPRI(CONF_facility, LOG_NOTICE),"SessionMgr::initializeSession - Menacon sid:%s\n",sid);
    }
    if(!sid || !sid[0]){
        base = sesfactory->createSession(0);
        base->setSID();
        if(CONF_facility)
            syslog(LOG_MAKEPRI(CONF_facility, LOG_NOTICE),"SessionMgr::initializeSession - No cookies persent. New sid:%s\n", base->sid);
    }
    else {
        ...
        if(sfile) {
            ...
            time_t now = time(0);
            if(CONF_facility)
                syslog(LOG_MAKEPRI(CONF_facility, LOG_NOTICE), "SessionMgr::initializeSession - Session age:%fh\n", (now-base->start)/60.0/60.0);
            if(now - base->start > CONF_max_age) {
                ...
                base->setSID();
                if(CONF_facility)
                    syslog(LOG_MAKEPRI(CONF_facility, LOG_NOTICE), "SessionMgr::initializeSession - session expired. New sid: %s\n", base->sid);
            }
            base->start = now;
        } else {
            base = sesfactory->createSession(0);
            base->setSID();
            if(CONF_facility)
                syslog(LOG_MAKEPRI(CONF_facility, LOG_NOTICE), "SessionMgr::initializeSession - File not found: %s - new: %s\n",sid, base->sid);
        }
    }
    req->app_data = base;
...
// -------------------------------------------------------------------------------------------------
void SessionMgr::closeSession(fcgi_driver::Request *req)
{
    ...
    if(base->isSID()) {
        ...
        if(!sfile && base->isSID()) {
            if(CONF_facility) syslog(LOG_MAKEPRI(CONF_facility, LOG_NOTICE), "SessionMgr::closeSession - Unable to store session with sid %s\n",base->sid);
            CS_VAPRT_ERRO("SessionMgr::closeSession - Unable to store session with sid %s",base->sid);
        }
        else{
    ...
    if(CONF_facility) syslog(LOG_MAKEPRI(CONF_facility, LOG_NOTICE),"SessionMgr::closeSession - time %ld\n",base->start);
    delete base;
    req->app_data = 0;
}
// -------------------------------------------------------------------------------------------------
int SessionMgr::login(fcgi_driver::Request *req)
{
...
// -------------------------------------------------------------------------------------------------
void SessionMgr::logout(fcgi_driver::Request *req)
{
    SessionBase *base = (SessionBase*) req->app_data;
    if(base->isSID()) {
...
// -------------------------------------------------------------------------------------------------
void SessionMgr::setLanguage(fcgi_driver::Request *req, const char *lang, bool set_session)
{
    int ndx;
    ...
    if(ndx==FRAME_LOCALES) {
        CS_VAPRT_WARN("SessionMgr::setLanguage - Language code %d not found from this system.",lcode);
        base->appstr = def_lang;
        base->locale_ndx = 0;
    }
}
// -------------------------------------------------------------------------------------------------
void SessionMgr::loadKeys()
{
    int *iptr;
    ...
    cs = open(CONF_session_dir.get_path().c_str(), O_CREAT|O_RDWR, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH);
    if(cs>0) {
        ...
        CS_VAPRT_WARN("SessionMgr::loadKeys - unable to open key count file %s. Errno %d",CONF_session_dir.get_path().c_str(),errno);
        count=0;
    }
    ...
    if((size_t)rb < FRAME_KEYS) {
        ...
        if(left < rb) {
            if(CONF_facility) syslog(LOG_MAKEPRI(CONF_facility, LOG_NOTICE),"SessionMgr::loadKeys - New keys loaded.\n");
...
RAND_KEYS:
    if(CONF_facility) syslog(LOG_MAKEPRI(CONF_facility, LOG_NOTICE),"SessionMgr::loadKeys - Generating random keys.\n");
    iptr = (int*) key_buf;
    srand(time(0));
...
void SessionMgr::purge()
{
    ...
    if(sessions.size()==0)
        return;
    if(CONF_facility) syslog(LOG_MAKEPRI(CONF_facility, LOG_NOTICE), "SessionMgr::purge - checking %ld session files.\n",sessions.size());
    time_t now = time(0);
    for(c4s::path_iterator sf = sessions.begin(); sf != sessions.end(); sf++) {
        fd = open(sf->get_path().c_str(), O_RDONLY);
        if(fd == -1) {
            CS_VAPRT_ERRO("SessionMgr::purge - Cannot open session file %s",sf->get_path().c_str());
            continue;
        }
        ...
        if(rv>0) {
            if(now - create > CONF_max_age) {
                if(CONF_facility) syslog(LOG_MAKEPRI(CONF_facility, LOG_NOTICE), "Purging stale session file %s\n", sf->get_base().c_str());
                sf->rm();
            }
        }
    }
...
// -------------------------------------------------------------------------------------------------
void SessionMgr::rmSessionFile(const char *sid)
{
    if(CONF_facility) syslog(LOG_MAKEPRI(CONF_facility, LOG_NOTICE), "SessionMgr::rmSessionFile - SID:%s\n", sid);
    CONF_session_dir.set_base(sid);
    CONF_session_dir.set_ext(".ses");
    if(CONF_session_dir.rm()==false) {
        CS_VAPRT_WARN("SessionMgr::rmSessionFile - Unable to remove session file with sid:%s", sid);
        if(CONF_facility) syslog(LOG_MAKEPRI(CONF_facility, LOG_NOTICE),"SessionMgr::rmSessionFile - remove failed");
    }
}
