...
// -------------------------------------------------------------------------------------------------
void Framework::create(const std::string &exe_dir, SessionFactoryIF *sf, c4s::configuration *cf)
{
    if(!the_frame)
        ...
        conf(conf)
{
    ...
    if(!sf || !conf)
        throw runtime_error("Framework::Framework - SessionFactory and configuration are required parameters.");
    ...
    if(!conf->get_value("LibFCGI","DirUpload",upload_dir)) {
        upload_dir = "upload/";
        syslog(LOG_DEBUG, "Framework::Framework - Missing DirUpload from configuration. Using default value.");
    }
    if(upload_dir[0] == '/')
...
Framework::~Framework()
{
    ...
    if(sesmgr) delete sesmgr;
}
// -------------------------------------------------------------------------------------------------
SessionBase* Framework::initializeRequest(fcgi_driver::Request *req)
{
    // Load or create session
    SessionBase *base = sesmgr->initializeSession(req);
    if(!base)
        throw runtime_error("Framework::initializeRequest - SessionBase cannot be null after initialization.");
...
// -------------------------------------------------------------------------------------------------
void Framework::beginRequest(fcgi_driver::Request *req, std::ostringstream &html, const char *type,
                             ostringstream *cookies, ostringstream *headers)
{
    ...
    if(!type)
        ...
        html<<"<html><head><title>Transfer</title></head><body><p>Redirecting...</p></body></html>\r\n";
        return;
    }
    else {
    ...
    if(base->isSID()) {
        CC cookie;
        if(base->old) cookie = base->dropCookieSID();
        else cookie = base->getCookieSID();
        html<<cookie;
        int facility = sesmgr->getFacility();
        if(facility)
            syslog(LOG_MAKEPRI(facility, LOG_NOTICE),
                   "Framework::beginRequest - %s", cookie);
    }
...
// -------------------------------------------------------------------------------------------------
void Framework::closeRequest(fcgi_driver::Request *req, uint32_t http_status)
{
    sesmgr->closeSession(req);
    req->end(http_status);
}
// -------------------------------------------------------------------------------------------------
void Framework::encodeURL(const char *origin, char *target, size_t tmax)
{
    uint8_t unsafe[15] = {
        0x20, 0x22, 0x3c, 0x3e, 0x23, 0x25, 0x7b,
        0x7d, 0x7c, 0x5c, 0x5e, 0x7e, 0x5b, 0x5d, 0x60
    };
...
// -------------------------------------------------------------------------------------------------
void Framework::decodeURL(char *URL)
{
    char *dc = URL;
    while(*URL) {
        if(*URL=='%') {
...
// -------------------------------------------------------------------------------------------------
void Framework::decodeURL(const char *URL, std::string &target)
{
    target.clear();
    while(*URL) {
        if(*URL=='%') {
...
// -------------------------------------------------------------------------------------------------
bool Framework::decodeDate(const char *date, tm *tptr)
{
    char *rv;
...
// -------------------------------------------------------------------------------------------------
const char* Framework::escapeHtml(const std::string &orig)
{
#if 0
...
// -------------------------------------------------------------------------------------------------
void Framework::escapeHtml(char *target, const char *orig)
{
    while(*orig) {
        if(*orig == '<') {
            strcpy(target,"&#60;");
...
// -------------------------------------------------------------------------------------------------
const char* Framework::escapeJson(const std::string &orig)
{
...
// -------------------------------------------------------------------------------------------------
bool Framework::moveUploadedFile(fcgi_driver::Request *req, const char *label, const char *target)
{
    char *pipe=0;
    if(!label || !target) {
        return false;
...
// -------------------------------------------------------------------------------------------------
int Framework::daemonize(const char *pid_fname)
